...
AST *parseRelExpr(Tokens &tokens) {
  ...
  while (token.isSymbol(eSymbol::Lt) || token.isSymbol(eSymbol::Le) ||
         token.isSymbol(eSymbol::Gt) || token.isSymbol(eSymbol::Ge)) {
    tokens.pop_front();
...
AST *parseFunction(Tokens &tokens) {
  ...
  if (token.isKeyword(eKeyword::Int)) {
    ...
    if (token.isSymbol(eSymbol::OpeningBrace)) {
      ...
      while (
          !(tokens.empty() || tokens.front().isSymbol(eSymbol::ClosingBrace))) {
